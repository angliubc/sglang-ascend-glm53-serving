...
class StrategyMap:
    strategy_map = {
        ...
        # 0911 ang: prefill alltoallv (aclnnNotifyDispatchA2 tiling-fails
        # cross-node, alltoallv passes) + decode LL=ops (0907 EP16 recipe;
        # LL=alltoallv measured 0.05 tok/s on EP32 4-node)
        ("alltoall_ops"): (
            NormalStrategy.ALLTOALL,
            LowLatencyStrategy.OPS,
        ),
        # 0911 ang: LL=ops aicore-exception'd (0x26) cross-node at DDT=256
        # (tiling passed, execution failed). LL=DEFAULT = deep_ep C++ native
        # (RDMA buffer + vendored MoeLowLatencyDispatchV2).
        ("alltoall_default"): (
            NormalStrategy.ALLTOALL,
            LowLatencyStrategy.DEFAULT,
        ),
